...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, PermissionFlagsBits, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
import { z } from 'zod';
...
export const MINECRAFT_RANK_VALUES = ['dueno', 'colider', 'miembro'];
export const CLAN_PLAYER_STATUS_VALUES = ['activo', 'ausente', 'inactivo', 'retirado'];
...
function getDiscordDefault(member, userId) {
...
function buildDefaultDraft(member, applicantUserId) {
    return {
...
function buildRankOptions(currentValue) {
...
function buildMinecraftRankLabel(value) {
    if (value === 'dueno')
        return 'Dueño';
    if (value === 'colider')
        return 'Colíder';
...
function buildMinecraftRankOptions(currentValue) {
...
function buildStatusOptions(currentValue) {
...
function withOptionalValue(input, value) {
    if (value.length > 0) {
...
function buildPreviewEmbed(session) {
    ...
    return buildRecruitmentNoticeEmbed({
        ...
        description: `Jugador: <@${session.applicantUserId}>\n` +
...
export async function canManageClanPlayers(member) {
...
export async function findAcceptedInterviewTicket(guildId, channelId) {
...
export function clearClanPlayerRegistrationSession(sessionId) {
    sessions.delete(sessionId);
}
export function buildClanPlayerBasicsModal(session) {
    return new ModalBuilder()
        .setCustomId(`${CLAN_PLAYER_CUSTOM_IDS.basicsModal}${session.id}`)
        .setTitle('Registrar jugador — datos base')
        .addComponents(new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('discord')
        .setLabel('Discord *')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(120), session.draft.discord)), new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('fullName')
        .setLabel('Nombre *')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(120), session.draft.fullName)), new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('minecraftNick')
        .setLabel('Nick Minecraft *')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(120), session.draft.minecraftNick)), new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('joinedAt')
        .setLabel('Fecha de ingreso *')
        .setPlaceholder('YYYY-MM-DD')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(10), session.draft.joinedAt)), new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('country')
        .setLabel('País *')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
...
export function buildClanPlayerAdditionalModal(session) {
    return new ModalBuilder()
        ...
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(20), session.draft.utcOffset)), new ActionRowBuilder().addComponents(withOptionalValue(new TextInputBuilder()
        .setCustomId('notes')
...
export function applyBasicsModalValues(sessionId, values) {
...
export function applyAdditionalModalValues(sessionId, values) {
    const session = getSession(sessionId);
    if (!session)
        return null;
...
export function applySessionSelectValue(sessionId, field, value) {
    const session = getSession(sessionId);
    if (!session)
        return null;
    if (field === 'rank') {
    ...
    else if (field === 'minecraftRank' && MINECRAFT_RANK_VALUES.includes(value)) {
        session.draft.minecraftRank = value;
    }
    else if (field === 'status' && CLAN_PLAYER_STATUS_VALUES.includes(value)) {
        session.draft.status = value;
    }
    setSession(session);
    return session;
}
export function buildClanPlayerStepOneReply(session) {
...
export function buildClanPlayerReviewReply(session) {
    const rankSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    const minecraftRankSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    const statusSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
...
export function getClanPlayerSession(sessionId) {
    return getSession(sessionId);
}
function toJoinedAtDate(value) {
...
export async function saveClanPlayerRegistration(sessionId) {
    const session = getSession(sessionId);
    if (!session) {
        return { ok: false, message: 'La sesión expiró. Ejecuta el comando otra vez.' };
    }
    ...
    if (discordTaken) {
        return { ok: false, message: 'Ya existe un jugador con ese Discord.' };
    }
    if (minecraftNickTaken) {
        return { ok: false, message: 'Ya existe un jugador con ese nick de Minecraft.' };
    }
    try {
...
//# sourceMappingURL=player-registration.js.map