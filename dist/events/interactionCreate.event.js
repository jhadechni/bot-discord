...
import { SUGGESTION_COLORS, buildSuggestionErrorEmbed, buildSuggestionNoticeEmbed, buildSuggestionPublicEmbed, buildSuggestionVoteButtons, } from '../utils/suggestion-ui.js';
...
import { getOrderFull, buildOrderEmbed, buildAcceptedButtons, buildInPreparationButtons, buildReadyButtons, buildPendingButtons, createPendingOrder, formatOrderStockStatusLine, getOrderStockAssessment, releaseOrderStock, consumeOrderStock, addSurchargeToOrder, } from '../shop/order-utils.js';
...
import { parseRejectionReasons, buildRejectionReasonSelect, buildRejectionConfirmRow, buildRejectionSelectorEmbed, pendingRecruitRejections, handleAddRejectionReasonModal, } from '../commands/apply/reclutamiento.command.js';
    ...
    try {
        if (channelId && messageId) {
            ...
            if (originalChannel?.isTextBased()) {
                ...
                const rejectedEmbed = buildRecruitmentRejectedApplicationEmbed(originalMsg.embeds[0] ?? new EmbedBuilder(), staffId, reason);
...
function parseApplyConfirmState(rawComponents) {
...
function formatDiscountLabel(discountType, value) {
...
function buildManualDiscountPrompt(params) {
    const embed = buildShopNoticeEmbed({
        ...
        fields: [{
                name: 'Opciones',
                value: params.options.slice(0, 25).map(option => `• **${option.productName}**: ${formatDiscountLabel(option.discountType, option.discountValue)} ` +
                    `(cantidad ${option.aggregatedQuantity}, tramo ${option.minQuantity}+)`).join('\n'),
            }],
    });
    const select = new StringSelectMenuBuilder()
        ...
        .addOptions(params.options.slice(0, 25).map(option => ({
...
async function refreshOrderManagementEmbeds(params) {
    ...
    if (!botUserId)
        return 0;
    const channelIds = Array.from(new Set((params.channelHints ?? []).filter((value) => !!value)));
    let updatedCount = 0;
    const componentRow = params.updatedOrder.status === 'accepted'
    ...
    for (const channelId of channelIds) {
        ...
        const matchingMessages = recentMessages.filter((message) => {
            if (message.author.id !== botUserId)
                return false;
            const hasMatchingTitle = message.embeds.some((embed) => (embed.title ?? '').includes(params.orderCode));
            const hasMatchingButton = message.components.some((row) => {
                if (!('components' in row) || !Array.isArray(row.components)) {
                    return false;
                }
                return row.components.some(component => 'customId' in component && typeof component.customId === 'string' && component.customId.includes(params.orderCode));
            });
        ...
        try {
            ...
            if (interaction.isChatInputCommand()) {
                ...
                try {
                    ...
                    const msg = {
                        embeds: [
                            buildSystemErrorEmbed('Error al ejecutar comando', 'Ocurrio un error al ejecutar este comando. Intentalo de nuevo.'),
                        ],
                        ephemeral: true,
                    };
            ...
            if (interaction.isModalSubmit() && interaction.customId === 'kit:create_modal') {
                ...
                if (!member)
                    return;
                const staffRoles = [guildConfig.staffRoleId, guildConfig.liderRoleId, guildConfig.coLiderRoleId].filter(Boolean);
                const isKitStaff = staffRoles.some(r => member.roles.cache.has(r)) || member.permissions.has('Administrator');
                if (!isKitStaff) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Permiso insuficiente', 'Solo el staff puede crear plantillas de kits.'),
                        ],
                    });
                    return;
                }
                ...
                if (!cooldownMin || cooldownMin < 1) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Cooldown inválido', 'Usa un formato como `7d`, `24h` o `3d12h`.'),
                        ],
                    });
                    return;
                }
                ...
                if (existing) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Plantilla duplicada', `Ya existe una plantilla llamada **${name}**.`),
                        ],
                    });
                    return;
                }
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'remind:kit:toggle') {
                ...
                const updatedSelect = new StringSelectMenuBuilder()
                    ...
                    .addOptions(templates.map(t => new StringSelectMenuOptionBuilder()
                    .setLabel(t.name)
                ...
                await interaction.editReply({
                    ...
                    components: [
                        ...
                        new ActionRowBuilder().addComponents(saveBtn, editBtn),
                    ],
                });
                return;
            }
            ...
            if (interaction.isButton() && interaction.customId === 'remind:kit:save') {
                ...
                const select = new StringSelectMenuBuilder()
                    ...
                    .addOptions(allTemplates.map(t => new StringSelectMenuOptionBuilder()
                    .setLabel(t.name)
                    .setDescription(t.description ?? `Cooldown: ${formatTime(t.cooldownMin)}`)
                    .setValue(t.id)
            ...
            if (interaction.isButton() && interaction.customId === 'remind:kit:edit_open') {
                ...
                const editSelect = new StringSelectMenuBuilder()
                    ...
                    .addOptions(activeReminders.map(r => {
                    const msLeft = r.triggerAt.getTime() - Date.now();
                    const timeLeft = msLeft > 0 ? formatTime(Math.ceil(msLeft / 60_000)) : 'vencido';
            ...
            if (interaction.isModalSubmit() && interaction.customId.startsWith('remind:kit:edit_modal:')) {
                ...
                if (!minutes || minutes < 5) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Tiempo inválido', 'El mínimo es 5 minutos. Usa un formato como `5m`, `2h` o `1d`.'),
                        ],
                    });
                    return;
                }
                ...
                if (!reminder) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Recordatorio no encontrado', 'No se encontró ese recordatorio o ya no está activo.'),
                        ],
                    });
                    return;
                }
            ...
            if (interaction.isButton() && interaction.customId.startsWith('remind:kit:claimed:')) {
                ...
                if (!template) {
                    await interaction.editReply({
                        embeds: [
                            buildReminderErrorEmbed('Plantilla no disponible', 'Esta plantilla de kit ya no está disponible.'),
                        ],
                    });
                    return;
                }
            ...
            if (interaction.isButton() && interaction.customId.startsWith('mod:logs:')) {
                ...
                if (!interaction.guildId || !targetId || isNaN(page))
                    return;
                ...
                await thread.send({
                    embeds: [
                        buildSuggestionNoticeEmbed({
                            ...
                            description: `<@${suggestion.userId}> lanzó esta sugerencia a la comunidad.\n\n` +
            ...
            if (interaction.isModalSubmit() && interaction.customId === 'suggest_modal') {
                ...
                if (banned) {
                    await interaction.editReply({
                        embeds: [
                            buildSuggestionErrorEmbed('Sugerencia bloqueada', 'Tu sugerencia contiene una palabra no permitida. Revisa el contenido e intenta de nuevo.'),
                        ],
                    });
                    return;
                }
                ...
                if (!config.suggestionsChannelId) {
                    await interaction.editReply({
                        embeds: [
                            buildSuggestionErrorEmbed('Canal no configurado', 'El canal de sugerencias no está configurado. Pide al staff que use `/config set-suggestions`.'),
                        ],
                    });
                    return;
                }
                ...
                if (!channel?.isTextBased()) {
                    await interaction.editReply({
                        embeds: [
                            buildSuggestionErrorEmbed('Canal inválido', 'El canal de sugerencias configurado no es válido.'),
                        ],
                    });
                    return;
                }
            ...
            if (interaction.isModalSubmit() && interaction.customId.startsWith('anunciar:modal:')) {
                ...
                while (restante.length > 2000) {
                    ...
                    if (corte < 1000)
                        corte = restante.lastIndexOf(' ', 2000);
                    if (corte < 1)
                        corte = 2000;
            ...
            if (interaction.isModalSubmit() && interaction.customId === 'tienda:catalog:request_modal') {
                ...
                const staffChannel = guild.channels.cache.get(config.shopStaffChannelId)
                    ?? await guild.channels.fetch(config.shopStaffChannelId).catch(() => null);
                if (!staffChannel?.isTextBased()) {
                    await interaction.editReply({
                        embeds: [
                            buildShopErrorEmbed('Canal staff inválido', 'El canal de staff configurado para la tienda no es válido.'),
                        ],
                    });
                    return;
                }
            ...
            function buildApplyPhaseOneReply(platform, role) {
                const isComplete = platform.length > 0 && role.length > 0;
                const platformRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                    .setCustomId('apply_platform_select')
                    .setPlaceholder('¿En qué plataforma juegas? (Java / Bedrock)')
                    .setMinValues(1)
                    .setMaxValues(1)
                    .addOptions(ALL_PLATFORMS.map(p => new StringSelectMenuOptionBuilder()
                    .setLabel(p.label)
                ...
                const roleRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                    ...
                    .setMinValues(1)
                    .setMaxValues(5)
                    .addOptions(ALL_APPLY_ROLES.map(r => new StringSelectMenuOptionBuilder()
                    .setLabel(r.label)
                ...
                return {
                    embeds: [embed],
                    components: [platformRow, roleRow, new ActionRowBuilder().addComponents(confirmBtn)],
                };
            }
            if (interaction.isStringSelectMenu() && interaction.customId === 'apply_platform_select') {
                ...
                const { role } = parseApplyConfirmState(interaction.message.components);
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'apply_role_select') {
                ...
                const { platform } = parseApplyConfirmState(interaction.message.components);
                await interaction.editReply(buildApplyPhaseOneReply(platform, role));
                return;
            }
            ...
            if (interaction.isModalSubmit() && interaction.customId.startsWith('apply_modal_')) {
                ...
                if (!/^\d{1,2}$/.test(age) || isNaN(ageNum) || ageNum < 1 || ageNum > 99) {
                    await interaction.editReply({
                        embeds: [
                            buildRecruitmentErrorEmbed('Edad no válida', 'Por favor introduce tu edad como un número entero (por ejemplo: `20`).'),
                        ],
                    });
                    return;
                }
                ...
                if (recruitBanned) {
                    await interaction.editReply({
                        embeds: [
                            buildRecruitmentErrorEmbed('Solicitud bloqueada', 'Tu solicitud contiene una palabra no permitida. Revisa tus respuestas e intenta de nuevo.'),
                        ],
                    });
                    return;
                }
            ...
            if (interaction.isModalSubmit() && interaction.customId.startsWith(CLAN_PLAYER_CUSTOM_IDS.basicsModal)) {
                ...
                if (!actor || !(await canManageClanPlayers(actor))) {
                    await interaction.reply({
                        embeds: [
                            buildRecruitmentErrorEmbed('Permiso insuficiente', 'Solo el staff puede registrar jugadores en el roster.'),
                        ],
                        ephemeral: true,
                    });
                    return;
                ...
                if (!session) {
                    await interaction.reply({
                        embeds: [
                            buildRecruitmentErrorEmbed('Sesión expirada', 'La sesión expiró. Ejecuta el comando otra vez.'),
                        ],
                        ephemeral: true,
                    });
                    return;
            ...
            if (interaction.isButton() && interaction.customId.startsWith('clanplayer:')) {
                ...
                const prefixToSlice = customId.startsWith(CLAN_PLAYER_CUSTOM_IDS.continueButton) ? CLAN_PLAYER_CUSTOM_IDS.continueButton :
                ...
                if (customId.startsWith(CLAN_PLAYER_CUSTOM_IDS.confirmButton)) {
                    ...
                    if (!result.ok) {
                        await interaction.reply({
                            embeds: [
                                buildRecruitmentErrorEmbed('No se pudo registrar', result.message),
                            ],
                            ephemeral: true,
                        });
                        return;
                    ...
                    const interviewChannel = guild.channels.cache.get(result.channelId) ??
            ...
            if (interaction.isButton() && interaction.customId.startsWith('apply_confirm_')) {
                ...
                if (!rolesJoined)
                    return; // estado deshabilitado, no debería llegar aquí
                const guildId = interaction.guildId;
                if (!guildId)
                    return;
                ...
                if (!platform || !role)
                    return; // estado incompleto, no debería llegar aquí
                const modal = new ModalBuilder()
                ...
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                    ...
                    .setRequired(true)
                    .setMinLength(1)
                    .setMaxLength(3)), new ActionRowBuilder().addComponents(new TextInputBuilder()
            ...
            if (interaction.isButton()) {
                ...
                if (customId.startsWith('apply_vote_approve_') || customId.startsWith('apply_vote_reject_')) {
                    ...
                    const eligibleRoles = [cfg.liderRoleId, cfg.coLiderRoleId, cfg.staffRoleId, cfg.reclutadorRoleId].filter(Boolean);
                ...
                if (customId.startsWith('apply_finalize_accept_')) {
                    ...
                    if (!isLider) {
                        const votes = await prisma.recruitmentVote.findMany({ where: { ticketId } });
                        const approveCount = votes.filter(v => v.vote === 'APPROVE').length;
                        const eligibleRolesForCount = [cfg.liderRoleId, cfg.coLiderRoleId, cfg.staffRoleId, cfg.reclutadorRoleId].filter(Boolean);
                        let eligibleCount = 0;
                        for (const roleId of eligibleRolesForCount) {
                            const role = guild.roles.cache.get(roleId) ?? await guild.roles.fetch(roleId).catch(() => null);
                            if (role)
                                eligibleCount += role.members.size;
                        }
                        const threshold = Math.floor(eligibleCount / 2) + 1;
                    ...
                    const acceptedEmbed = buildRecruitmentAcceptedApplicationEmbed(interaction.message.embeds[0] ?? new EmbedBuilder(), interaction.user.id);
                                ...
                                await privateChannel.send({
                                    embeds: [
                                        buildRecruitmentNoticeEmbed({
                                            ...
                                            description: `Solicitud aceptada por <@${interaction.user.id}>.\n\n` +
                ...
                if (customId.startsWith('apply_finalize_reject_') && !customId.startsWith('apply_finalize_reject_reason_')) {
                    ...
                    if (rejectionReasons.length > 0) {
                        ...
                        await interaction.reply({
                            ...
                            components: [
                                new ActionRowBuilder().addComponents(buildRejectionReasonSelect(rejectionReasons, ticketId, chId, msgId)),
                        ...
                        rejectModal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
            ...
            if (interaction.isButton() && interaction.customId.startsWith('shop:')) {
                ...
                const hasPermission = actor.permissions.has(PermissionFlagsBits.ManageGuild) ||
                            ...
                            await ticketCh.send({
                                ...
                                embeds: [
                                    buildShopNoticeEmbed({
                                        ...
                                        description: `👋 ¡Hola <@${order.customer.discordUserId}>! Tu pedido **${orderCode}** fue aceptado y está en cola.\n` +
                    ...
                    try {
                        ...
                        await customerMember.user.send({
                            embeds: [
                                buildShopNoticeEmbed({
                                    title: 'Pedido aceptado',
                                    description: `Tu pedido **${orderCode}** en **${guild.name}** fue aceptado.` +
                            ...
                            await ticketCh.send({
                                ...
                                embeds: [
                                    buildShopNoticeEmbed({
                                        ...
                                        description: `🔧 <@${order.customer.discordUserId}> ¡Tu pedido **${orderCode}** está siendo preparado!\n` +
                        ...
                        if (ticketCh?.isTextBased()) {
                            await ticketCh.send({
                                ...
                                embeds: [
                                    buildShopNoticeEmbed({
                                        title: '🔷 Pedido listo',
                                        description: `📦 <@${order.customer.discordUserId}> ¡Tu pedido **${orderCode}** está listo!\n` +
                ...
                if (action === 'add_service') {
                    ...
                    serviceModal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                        .setCustomId('service_label')
                        .setLabel('Nombre del servicio (ej: Domicilio)')
                        .setStyle(TextInputStyle.Short)
                        .setRequired(true)
                        .setMaxLength(80)), new ActionRowBuilder().addComponents(new TextInputBuilder()
                ...
                if (action === 'close') {
                    ...
                    try {
                        ...
                        await interaction.followUp({
                            embeds: [
                                buildShopErrorEmbed('No se puede entregar', [
                            ...
                            await ticketCh.send({
                                ...
                                embeds: [
                                    buildShopNoticeEmbed({
                                        ...
                                        description: `🎉 <@${order.customer.discordUserId}> ¡Tu pedido **${orderCode}** fue entregado!\n` +
                ...
                if (action === 'cancel') {
                    ...
                    cancelModal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
            ...
            if (interaction.isStringSelectMenu() && interaction.customId.startsWith('clanplayer:')) {
                ...
                const mapping = [
                    { prefix: CLAN_PLAYER_CUSTOM_IDS.rankSelect, field: 'rank' },
                    { prefix: CLAN_PLAYER_CUSTOM_IDS.minecraftRankSelect, field: 'minecraftRank' },
                    { prefix: CLAN_PLAYER_CUSTOM_IDS.statusSelect, field: 'status' },
            ...
            if (interaction.isStringSelectMenu() && interaction.customId.startsWith('shop:discount_select:')) {
                ...
                try {
                    ...
                    await interaction.editReply({
                        embeds: [
                            buildShopNoticeEmbed({
                                ...
                                description: [
                                    ...appliedDiscounts.map(discount => `• ${discount.productName}: ${formatDiscountLabel(discount.discountType, discount.discountValue)} (${discount.minQuantity}+)`),
                    ...
                    await interaction.editReply({
                        embeds: [
                            buildShopErrorEmbed('No se pudieron aplicar descuentos', err instanceof Error ? err.message : 'No se pudieron aplicar los descuentos.'),
                        ],
                    });
                }
                return;
            ...
            if (interaction.isButton() && interaction.customId.startsWith('tienda:catalog:')) {
                ...
                const { embed, components } = buildCatalogView(products, mode, category, subcategory, Number.isNaN(requestedPage) ? 1 : requestedPage);
                await interaction.editReply({ embeds: [embed], components });
                return;
            }
            ...
            if (interaction.isModalSubmit() && interaction.customId === 'pedido:cart:qty_modal') {
                ...
                if (!hasProductInventoryDefinition(product)) {
                    await interaction.reply({
                        embeds: [
                            buildShopErrorEmbed('Producto no vendible', `**${product.name}** no tiene una definición de inventario válida y no se puede vender todavía.`),
                        ],
                        ephemeral: true,
                    });
                    return;
            ...
            if (interaction.isButton() && interaction.customId === 'pedido:cart:confirm') {
                ...
                try {
                    ...
                    await interaction.followUp({
                        embeds: [
                            buildShopErrorEmbed('No se pudo crear el pedido', err instanceof Error ? err.message : 'No se pudo crear el pedido.'),
                        ],
                        ephemeral: true,
                    });
                    return;
            ...
            if (interaction.isModalSubmit() && (interaction.customId.startsWith('apply_finalize_reject_reason_') ||
                ...
                try {
                    ...
                    if (originalChannel?.isTextBased()) {
                        const originalMsg = await originalChannel.messages.fetch(messageId);
                        const rejectedEmbed = buildRecruitmentRejectedApplicationEmbed(originalMsg.embeds[0] ?? new EmbedBuilder(), interaction.user.id, reason);
                        await originalMsg.edit({ embeds: [rejectedEmbed], components: [] });
                    }
                }
                catch (err) {
                        ...
                        await ticketCh.send({
                            ...
                            embeds: [
                                buildShopNoticeEmbed({
                                    ...
                                    description: `😔 <@${order.customer.discordUserId}> tu pedido **${orderCode}** fue cancelado.\n` +
            ...
            if (interaction.isButton() && interaction.customId === 'notif:open') {
                ...
                const memberRoles = interaction.member && 'cache' in interaction.member.roles
                    ? interaction.member.roles.cache
                    : null;
                const currentRoleIds = new Set(notifRoles.map(r => r.roleId).filter(id => memberRoles?.has(id) ?? false));
            ...
            if (interaction.isStringSelectMenu() && interaction.customId.startsWith('recruit:reason_select:')) {
                ...
                await interaction.editReply({
                    ...
                    components: [
                        new ActionRowBuilder().addComponents(buildRejectionReasonSelect(reasons, ticketId, channelId, messageId, selected)),
            ...
            try {
                const msg = {
                    embeds: [
                        buildSystemErrorEmbed('Error interno', 'Ocurrio un error interno. Intentalo de nuevo.'),
                    ],
                    ephemeral: true,
                };
