...
import { buildAutomodFilterDmEmbed, buildAutomodFilterLogEmbed, buildAutomodSpamDmEmbed, buildAutomodSpamLogEmbed, buildAutomodSpamPublicEmbed, buildAutomodToxicityDmEmbed, buildAutomodToxicityLogEmbed, } from '../utils/automod-ui.js';
...
function getSpamEntry(key) {
    if (!spamMap.has(key))
        spamMap.set(key, { timestamps: [], mentions: [] });
    return spamMap.get(key);
}
function pruneOld(arr, window) {
    const cutoff = Date.now() - window;
    return arr.filter(t => t > cutoff);
}
// ── XP cooldown (en memoria) ──────────────────────────────────────────────────
const xpCooldown = new Map(); // key: guildId-userId → last XP timestamp
async function trackMessageXp(message, guildId, userId, config) {
...
const messageCreateEvent = {
    ...
    async execute(message) {
        ...
        const hasModerationBypass = member.permissions.has('Administrator') ||
        ...
        try {
            ...
            const toDelete = [...recent.values()].filter(m => m.author.id === userId && Date.now() - m.createdTimestamp < 30_000);
