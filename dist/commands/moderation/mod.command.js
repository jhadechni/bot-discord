...
function resolvePermissionLabel(permission) {
...
async function ensureSubcommandPermission(interaction, group, subcommand) {
    ...
    await interaction.reply({
        embeds: [
            buildModerationErrorEmbed('Permiso insuficiente', `Necesitas el permiso **${resolvePermissionLabel(requiredPermission)}** para usar este subcomando.`),
        ],
        ephemeral: true,
    });
    return false;
...
export const modCommand = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(opt => opt.setName('usuario').setDescription('Usuario al que pertenece la advertencia').setRequired(true))
        .addStringOption(opt => opt.setName('id').setDescription('ID de la advertencia (visible en /warnings)').setRequired(true))
        .addStringOption(opt => opt.setName('motivo').setDescription('Motivo de la eliminación').setRequired(false)))
        .addSubcommand(sub => sub
        .setName('kick')
        ...
        .addUserOption(opt => opt.setName('usuario').setDescription('Usuario a banear').setRequired(true))
        .addIntegerOption(opt => opt.setName('duracion').setDescription('Duración en minutos').setRequired(true).setMinValue(1))
        .addStringOption(opt => opt.setName('motivo').setDescription('Motivo del baneo').setRequired(false)))
        .addSubcommand(sub => sub
        .setName('timeout')
        ...
        .addStringOption(opt => opt.setName('id').setDescription('ID de la acción (obtenido con /mod historial logs)').setRequired(true))
        ...
        .addStringOption(opt => opt.setName('palabra').setDescription('Palabra a filtrar').setRequired(true).setMaxLength(100)))
        .addSubcommand(sub => sub
        .setName('eliminar')
        .setDescription('Elimina una palabra del filtro')
        .addStringOption(opt => opt.setName('palabra').setDescription('Palabra a eliminar').setRequired(true)))
        .addSubcommand(sub => sub.setName('lista').setDescription('Muestra todas las palabras filtradas'))),
    async execute(interaction) {
        ...
        await interaction.reply({
            embeds: [
                buildModerationErrorEmbed('Subcomando no soportado', 'Este subcomando de moderación no está disponible.'),
            ],
            ephemeral: true,
        });
    },
