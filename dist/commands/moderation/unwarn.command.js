...
import { MODERATION_COLORS, buildModerationLogEmbed, buildModerationStaffEmbed, buildModerationErrorEmbed, } from '../../utils/moderation-ui.js';
    ...
    async execute(interaction) {
        ...
        if (!warn) {
            await interaction.editReply({
                embeds: [
                    buildModerationErrorEmbed('Advertencia no encontrada', 'No existe una advertencia activa con ese ID para ese usuario.\nUsa `/warnings` para ver los IDs correctos.'),
                ],
            });
            return;
        }
            ...
            await logsChannel.send({
                embeds: [
                    buildModerationLogEmbed({
                        ...
                        logId: warn.id,
                        createdAt: new Date(),
                    }).addFields({ name: 'Advertencia original', value: warn.reason ?? 'Sin motivo', inline: true }, { name: 'Advertencias restantes', value: `${remaining}`, inline: true }),
                ],
            });
        }
        await interaction.editReply({
...
//# sourceMappingURL=unwarn.command.js.map