    ...
    async execute(interaction) {
        ...
        if (!target || !('moderatable' in target) || !target.moderatable) {
            await interaction.editReply({
                embeds: [
                    buildModerationErrorEmbed('⚠️ No se pudo aplicar el silencio', 'No puedo silenciar a ese usuario por jerarquía, permisos o protección interna.'),
                ],
            });
            return;
        }
...
export const tempmuteCommand = {
    ...
    async execute(interaction) {
        ...
        if (!target || !('moderatable' in target) || !target.moderatable) {
            await interaction.editReply({
                embeds: [
                    buildModerationErrorEmbed('⚠️ No se pudo aplicar el silencio temporal', 'No puedo silenciar temporalmente a ese usuario por jerarquía, permisos o protección interna.'),
                ],
            });
            return;
        }
...
export const unmuteCommand = {
    ...
    async execute(interaction) {
        ...
        if (!target || !('moderatable' in target) || !target.moderatable) {
            await interaction.editReply({
                embeds: [
                    buildModerationErrorEmbed('⚠️ No se pudo retirar el silencio', 'Ese usuario no tiene un silencio activo o no puedo modificar su estado.'),
                ],
            });
            return;
        }
...
//# sourceMappingURL=mute.command.js.map