...
function buildFilterWordPreview(words) {
    const fields = [];
    let current = [];
    let currentLength = 0;
    let shown = 0;
    ...
    async execute(interaction) {
        ...
        if (sub === 'eliminar') {
            ...
            if (deleted.count === 0) {
                await interaction.editReply({
                    embeds: [
                        buildModerationErrorEmbed('Palabra no encontrada', `\`${word}\` no estaba registrada en el filtro.`),
                    ],
                });
            }
            else {
        ...
        await interaction.editReply({
            embeds: [
                buildModerationNoticeEmbed({
                    title: '🚫 Palabras filtradas',
                    description: `Total registradas: **${words.length}**.` +
...
//# sourceMappingURL=filter.command.js.map