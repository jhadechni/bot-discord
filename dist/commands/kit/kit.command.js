...
import { REMINDER_COLORS, buildReminderErrorEmbed, buildReminderNoticeEmbed, } from '../../utils/reminder-ui.js';
async function hasStaffAccess(guildId, userId, guild) {
    const config = await getOrCreateGuildConfig(guildId);
    try {
        const member = await guild.members.fetch(userId);
        const staffRoles = [config.staffRoleId, config.liderRoleId, config.coLiderRoleId].filter(Boolean);
...
export const kitCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('crear').setDescription('Crear una nueva plantilla de kit'))
        .addSubcommand(sub => sub.setName('lista').setDescription('Ver todas las plantillas configuradas'))
    ...
    async execute(interaction) {
        ...
        if (!(await hasStaffAccess(guildId, interaction.user.id, guild))) {
            await interaction.reply({
                embeds: [
                    buildReminderErrorEmbed('Permiso insuficiente', 'Solo el staff puede gestionar las plantillas de kits.'),
                ],
                ephemeral: true,
            });
            return;
        ...
        if (sub === 'crear') {
            ...
            modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
                ...
                .setRequired(true)
                .setMaxLength(50)), new ActionRowBuilder().addComponents(new TextInputBuilder()
        ...
        if (sub === 'eliminar') {
            ...
            if (!template) {
                await interaction.editReply({
                    embeds: [
                        buildReminderErrorEmbed('Plantilla no encontrada', `No se encontró ninguna plantilla llamada **${nombre}**.`),
                    ],
                });
                return;
            }
...
//# sourceMappingURL=kit.command.js.map