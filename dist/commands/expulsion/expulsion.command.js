...
// ── Session en RAM ────────────────────────────────────────────────────────────
export const pendingExpulsions = new Map();
// ── Helpers ───────────────────────────────────────────────────────────────────
export function parseExpulsionReasons(raw) {
...
export function buildConfirmRow(targetId, disabled = true) {
...
export const expulsionCommand = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('usuario').setDescription('Jugador Aquaris a expulsar').setRequired(true).setAutocomplete(true)))
        ...
        .addStringOption(opt => opt.setName('id').setDescription('ID del motivo (visible en /expulsion motivo ver)').setRequired(true)))
    ...
    async execute(interaction) {
        ...
        if (sub === 'ejecutar') {
            ...
            const embed = buildExpulsionSelectorEmbed({ id: target.id, username: target.username, displayName: targetMember.displayName }, reasons);
            const selectRow = new ActionRowBuilder().addComponents(buildReasonSelectMenu(reasons, target.id));
            await interaction.editReply({
                embeds: [embed],
    ...
    async autocomplete(interaction) {
        ...
        const results = role.members
            ...
            .first(25);
        await interaction.respond(results.map(m => ({
            name: `${m.displayName} (${m.user.username})`,
            value: m.id,
        })));
    },
};
//# sourceMappingURL=expulsion.command.js.map