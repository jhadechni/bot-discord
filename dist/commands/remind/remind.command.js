import { SlashCommandBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, } from 'discord.js';
...
export const remindCommand = {
    data: new SlashCommandBuilder()
        ...
        .setRequired(false)))
        .addSubcommand(sub => sub.setName('lista').setDescription('Ver tus recordatorios activos'))
        .addSubcommand(sub => sub
        .setName('cancelar')
        ...
        .setRequired(false)))
        .addSubcommand(sub => sub.setName('kit').setDescription('Gestionar recordatorios de kits del servidor')),
    async execute(interaction) {
        ...
        if (sub === 'add') {
            ...
            if (activeCount >= 10) {
                await interaction.editReply({
                    embeds: [
                        buildReminderErrorEmbed('Límite alcanzado', 'Ya tienes 10 recordatorios activos. Cancela alguno antes de crear uno nuevo.'),
                    ],
                });
                return;
            }
        ...
        if (sub === 'cancelar') {
            ...
            if (!reminder) {
                await interaction.editReply({
                    embeds: [
                        buildReminderErrorEmbed('Recordatorio no encontrado', 'Usa `/remind lista` para ver tus IDs activos.'),
                    ],
                });
                return;
            }
        ...
        if (sub === 'editar') {
            ...
            if (!nuevoMensaje && !nuevoTiempo && repetir === null) {
                await interaction.editReply({
                    embeds: [
                        buildReminderErrorEmbed('Sin cambios', 'Debes especificar al menos un campo a editar: `mensaje`, `tiempo` o `repetir`.'),
                    ],
                });
                return;
            }
            ...
            const cambios = [];
            if (nuevoMensaje)
                cambios.push(`mensaje → "${nuevoMensaje}"`);
            if (nuevoTiempo)
                cambios.push(`tiempo → ${formatTime(parseTime(nuevoTiempo))}`);
            if (repetir === true)
                cambios.push('repetición activada');
            if (repetir === false)
                cambios.push('repetición desactivada');
            await interaction.editReply({
                embeds: [
                    buildReminderNoticeEmbed({
        ...
        if (sub === 'kit') {
            if (!interaction.guildId) {
                await interaction.editReply({
                    embeds: [
                        buildReminderErrorEmbed('Servidor requerido', 'Este subcomando solo funciona en un servidor.'),
                    ],
                });
                return;
            }
...
//# sourceMappingURL=remind.command.js.map