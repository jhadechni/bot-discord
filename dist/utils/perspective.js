 ...
 */
export async function analyzeToxicity(text, apiKey) {
    if (text.length < 8)
        return null; // Ignorar mensajes muy cortos
    const now = Date.now();
    if (now - lastRequestTime < MIN_INTERVAL_MS)
        return null; // Rate-limit local
    lastRequestTime = now;
    try {
        ...
        if (!response.ok) {
            if (response.status === 429)
                logger.warn('[perspective] Rate limit alcanzado en la API');
            return null;
        }
