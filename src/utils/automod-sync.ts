 ...
 */
export async function syncAutoModRule(client: Client, guildId: string): Promise<void> {
  ...
  try {
    ...
    if (words.length > MAX_KEYWORDS) {
      logger.warn(
        `[automod] Guild ${guildId}: ${words.length} palabras exceden el límite de ${MAX_KEYWORDS}. Se usarán las primeras ${MAX_KEYWORDS}.`,
      );
    }
