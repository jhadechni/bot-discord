...
export function buildWelcomePublicEmbed(options: MemberMessageOptions) {
  return buildAquarisEmbed({
    ...
    description:
      `¡Hola <@${options.userId}>, bienvenido(a) a 👋  **${options.guildName}**!\n\n` +
...
export function buildFarewellPublicEmbed(options: MemberMessageOptions) {
  return buildAquarisEmbed({
    description:
      `<@${options.userId}> ya no forma parte de ${options.guildName} <:salida:1506691970175733891>\n\n` +
