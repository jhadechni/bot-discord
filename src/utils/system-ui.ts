...
function rawId(id: string | null | undefined): string {
  return id ? `\`${id}\`` : 'No configurado';
}

export function buildConfigOverviewEmbeds(config: GuildConfig) {
  return [
    buildAquarisEmbed({
...
export function buildSystemNoticeEmbed(title: string, description: string, color = SYSTEM_COLORS.success) {
...
export function buildSystemErrorEmbed(title: string, description: string) {
  return buildAquarisErrorEmbed(title, description);
}

export function buildHelpEmbed(isStaff: boolean) {
...
export function buildPingEmbed(wsLatency: number, dbStatus: string) {
  return buildAquarisEmbed({
