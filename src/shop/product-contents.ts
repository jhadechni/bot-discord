...
export function getVisibleProductComponents(product: ProductLike): ComponentLike[] {
...
export function buildProductContentsSummary(
  product: ProductLike,
  maxItems = 4,
): string | null {
  ...
  const visible = components.slice(0, maxItems).map(component =>
    `${component.material.name} x${component.quantityRequired}`,
  );
...
export function buildProductContentsLines(
  product: ProductLike,
  maxItems = 6,
): string[] {
  const components = getVisibleProductComponents(product);
