...
export const solicitudesCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt
        ...
        .setRequired(false)
        .addChoices(
          { name: 'Pendientes (sin revisar)', value: 'OPEN' },
          { name: 'En entrevista', value: 'ACCEPTED' },
          { name: 'Todas las activas', value: 'ALL' },
        ),
    )
  ...
  async execute(interaction) {
    ...
    const recruitRoles = [cfg.liderRoleId, cfg.coLiderRoleId, cfg.staffRoleId, cfg.reclutadorRoleId].filter(Boolean) as string[];
    ...
    const statusFilter =
      estadoOpt === 'ALL' ? { in: ['OPEN', 'ACCEPTED'] } : estadoOpt;
