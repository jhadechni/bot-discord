...
export const pendingRecruitRejections = new Map<string, {
  ticketId: string;
  channelId: string;
  messageId: string;
  label: string;
...
export function parseRejectionReasons(raw: unknown): RecruitRejectionReason[] {
  if (!Array.isArray(raw)) return [];
  return raw.filter(
    (r): r is RecruitRejectionReason =>
...
export function buildRejectionReasonSelect(reasons: RecruitRejectionReason[], ticketId: string, channelId: string, messageId: string, selectedId?: string) {
  const options = [
    ...reasons.map(r =>
...
export function buildRejectionConfirmRow(ticketId: string, channelId: string, messageId: string, disabled = true) {
...
export function buildRejectionSelectorEmbed(reasons: RecruitRejectionReason[], selectedLabel?: string): EmbedBuilder {
...
export const reclutamientoCommand: Command = {
      ...
      group
        ...
        .addSubcommand(sub =>
          sub
            ...
            .addStringOption(opt =>
              opt.setName('id').setDescription('ID del motivo (visible en /reclutamiento motivo ver)').setRequired(true),
            ),
        )
        .addSubcommand(sub =>
          ...
          .addComponents(
            new ActionRowBuilder<TextInputBuilder>().addComponents(
              new TextInputBuilder()
                ...
                .setRequired(true)
                .setMaxLength(80)
                .setPlaceholder('Ej: No cumple los requisitos'),
            ),
            new ActionRowBuilder<TextInputBuilder>().addComponents(
              new TextInputBuilder()
                .setCustomId('body')
...
export async function handleAddRejectionReasonModal(
  ...
  const newReason: RecruitRejectionReason = { id: randomUUID().slice(0, 8), label, body };
