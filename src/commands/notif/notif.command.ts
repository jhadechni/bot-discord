...
export function parseNotifRoles(raw: unknown): NotifRole[] {
...
export function buildNotifPanel(roles: NotifRole[]): {
  embeds: EmbedBuilder[];
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      'Selecciona los temas que te interesan y recibirás pings cuando haya novedades.\n' +
...
export function buildNotifEphemeral(
  roles: NotifRole[],
...
export const notifCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addRoleOption(opt =>
          opt.setName('rol').setDescription('Rol de Discord a agregar').setRequired(true),
        )
        .addStringOption(opt =>
          opt.setName('emoji').setDescription('Emoji del rol (ej: 🛒)').setRequired(true),
        )
        .addStringOption(opt =>
          opt
            .setName('descripcion')
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addRoleOption(opt =>
          opt.setName('rol').setDescription('Rol a eliminar del panel').setRequired(true),
        ),
    )
    .addSubcommand(sub =>
      sub
  ...
  async execute(interaction) {
    ...
    if (sub === 'refresh') {
      const updated = await refreshPanel(
        guildId,
        roles,
        config.notifPanelChannelId,
        config.notifPanelMessageId,
        interaction.client,
      );
